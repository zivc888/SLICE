...
DD_profile <- function(base_mat, mut_mat, cells, only_target = F, return_p_value_for_gene = c(F,NA)){
  ...
  for(gene in genes){
    ...
    df <- gene_stat %>%
      ...
      mutate(
        wilcox_test = map2(no, yes, ~{wilcox.test(.x$dependency, .y$dependency, exact = FALSE) %>%
            broom::tidy()}),
        median_diff = map2_dbl(no, yes, ~{median(.y$dependency, na.rm = TRUE) - median(.x$dependency, na.rm = TRUE)}),
        no = map(no, nrow),
        yes = map(yes, nrow)
      ) %>% 
      unnest()
    ...
    tmp <- df[,c('median_diff','qvalue')]

    tmp[-log10(tmp$qvalue) < 2,]$median_diff <- 0
    # Insignificant cells (-log10(PFDR) < 2) are considered 0.
    tmp[tmp$median_diff < 0.2,]$median_diff <- 0        
    # All values below 0.2 are considered 0.
    
    base_mat[gene,] <- t(tmp$median_diff)
  }
...
enrichment_analysis <- function(GOIs, current_depmap_ids, approved_genes) {
  ...
  valid_approved_genes <- intersect(approved_genes, rownames(tmp))
  cat("Using", length(valid_approved_genes), "valid approved genes\n")
  
  # Use only valid genes
  if(length(valid_approved_genes) > 0) {
    tmp <- tmp[valid_approved_genes,]
    ...
    # Create all combinations of gene sets, cell lines and genes of interest
    ...
    # If no valid genes, return empty dataframe
    cat("ERROR: No valid approved genes found in the expression matrix!\n")
    return(data.frame(gene_set = character(), 
                     gene_name = character(),
                     Correlation = numeric(),
                     P_Value = numeric()))
  }
